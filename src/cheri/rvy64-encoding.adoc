...
| AP_MAX      | 0x2F                 | Value of the <<AP-field>> giving maximum permissions
...
That is, invert the most significant bit of _t_ if the decoded top and base are more than one address space (2^XLEN^ bytes) apart, i.e., if the decoded length would otherwise be larger than 2^XLEN^.
